...
- **Гардарика** — исходники игры вынесены в соседний репозиторий `../gardarika-game`. Править игру там; для публикации свежей сборки в этот сайт запускать из `gardarika-game` команду `npm run build:site` (или задать `GARDARIKA_SITE_DIR=/absolute/path/to/neuro-slop.ru/dev/public/lab/gardarika`). В этом репозитории остаётся только готовая статика `dev/public/lab/gardarika/` и пост `dev/src/content/posts/gardarika.md`.
