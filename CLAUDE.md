...
Сайт-лаборатория экспериментов с нейросетями: игры, сгенерированные нейросетями, и новости. Статика на Astro. Исходники — в `dev/`, собранный сайт лежит в корне репозитория и раздаётся GitHub Pages из ветки `main`.

## Контент

- **Записи** — markdown-файлы в `dev/src/content/posts/`. Фронтматтер: `title`, `description`, `date`, `category` (`news` или `games`), `cover` (путь к фоновой картинке — у каждой записи должна быть) и опционально `draft: true`. Имя файла = слаг URL (`/posts/<имя>/`), писать его латиницей. Черновики (`draft: true`) видны в `npm run dev`, но не попадают в сборку.
